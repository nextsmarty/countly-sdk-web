...
(function (root, factory) {
    ...
    Countly.session_duration = function (sec) {
        if (Countly.check_consent("sessions")) {
            if (sessionStarted) {
                ...
                toRequestQueue({ session_duration: sec, _events: sentEventList });
            }
        }
    };
    ...
    **/
    ...
    function add_cly_events(event) {
        ...
        e.dow = date.getDay();
        e._id = hashCode(e.key + e.timestamp + e.segmentation.url)
        eventQueue.push(e);
        sentEventList.push(e._id)
    ...
    **/
    Countly.track_sessions = function (inactive=false) {
        //start session
    ...
    **/
    //Countly.Minty = {}
    Countly.minty_track_pageview = function(page_name, properties) {
      properties = properties || {}
      page_name = page_name || window.location.pathname
      properties.id = page_name
      properties.url = properties.url || window.location.href
      //key = 'view'
      lastView = page_name
      if (typeof document.referrer !== "undefined" && document.referrer.length){
        properties.ref = document.referrer
      }
      add_cly_events({
          "key": "pageview",
    ...
    Countly.minty_track_itemview = function(html_element, properties) {
      properties = properties || {}

      var class_name = html_element.className
      properties.id = hashCode(window.location.pathname + class_name + html_element.title)
      properties.url = properties.url || window.location.href
      if (typeof document.referrer !== "undefined" && document.referrer.length){
        properties.ref = document.referrer
      }
    ...
    Countly.minty_track_user_action = function(action_name, properties){
      properties = properties || {}
